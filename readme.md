...
- [섹션 3. 스레드 생성과 실행](#섹션-3-스레드-생성과-실행)
- [섹션 4. 스레드 제어와 생명 주기 1](#섹션-4-스레드-제어와-생명-주기1)
- [섹션 5. 스레드 제어와 생명 주기 2](#섹션-5-스레드-제어와-생명-주기2)
- [섹션 6. 메모리 가시성](#섹션-6-메모리-가시성)
- [섹션 7. 동기화---synchronized](#섹션-7-동기화---synchronized)
- [섹션 8. 고급 동기화 - javautilconcurrentlocks](#섹션-8-고급-동기화---javautilconcurrentlocks)
- [섹션 9. 생산자 소비자 문제 1](#섹션-9-생산자-소비자-문제-1)
- [섹션 10. 생산자 소비자 문제 2](#섹션-10-생산자-소비자-문제2)
- [섹션 11. CAS - 동기화와 원자적 연산](#섹션-11-cas---동기화와-원자적-연산)
- [섹션 12. 동시성 컬렉션](#섹션-12-동시성-컬렉션)
- [섹션 13. 스레드 풀과 Executor 프레임워크 1](#섹션-13-스레드-풀과-executor-프레임워크1)
- [섹션 14. 스레드 풀과 Executor 프레임워크 2](#섹션-14-스레드-풀과-executor-프레임워크2)
...
### 1. 쓰레드 생성 비용으로 인한 성능 문제

* 쓰레드는 무거운 자원임
* 생성 시 다음과 같은 자원 소모됨

  * **메모리 할당**: 각 쓰레드는 고유한 호출 스택 필요 → 생성 시 메모리 소모
  * **운영체제 자원 사용**: 쓰레드 생성은 커널 수준에서 시스템 콜이 필요
  * **운영체제 스케줄러 설정**: 새 쓰레드는 OS 스케줄러가 관리 → 오버헤드 발생
  * **참고**: 쓰레드 하나당 보통 1MB 이상 메모리 사용

### 2. 쓰레드 관리 문제

* 서버의 CPU, 메모리는 유한함 → 쓰레드를 무한히 만들 수 없음

### 3. Runnable 인터페이스의 불편함

* **반환값 없음**: `run()` 메서드는 값을 반환하지 않음
* **체크 예외 처리 불가**: `run()` 메서드는 체크 예외를 던질 수 없음 → 내부에서 직접 처리해야 함

---

## Executor 프레임워크 소개

* 자바에서 멀티쓰레딩과 병렬 처리를 쉽게 하기 위해 제공되는 프레임워크임
* 쓰레드 생성 및 실행 관리 기능 제공
* 개발자가 직접 쓰레드 관리하지 않아도 됨
...
| 메서드   | `void run()`  | `V call()`            |
| 반환값   | 없음            | 있음 (제네릭 V)            |
| 예외 처리 | 체크 예외 던질 수 없음 | `throws Exception` 가능 |

---

## Callable, Future 도입 이유

* Runnable은 반환값, 예외 처리 모두 불편함
* Callable, Future를 통해 작업 결과 받거나 예외 처리 가능하게 함

---

## Future

* `submit()` 호출 시 작업 결과 대신 **Future 객체**를 반환함
* 실제 작업은 쓰레드 풀의 쓰레드가 수행하고, 결과는 나중에 받아옴

### 특징

* 요청 쓰레드는 `submit()` 호출 시 블로킹되지 않음
* 작업 결과 필요할 때 `future.get()` 호출
* 작업 완료 시: 즉시 결과 반환
* 작업 미완료 시: get() 호출 쓰레드는 대기 (블로킹)
...
| 메서드               | 설명                                                                            |
| ----------------- | ----------------------------------------------------------------------------- |
| `cancel(boolean)` | 실행 전 or 실행 중 작업을 취소. true이면 실행 중인 작업도 인터럽트 시도                                 |
| `isCancelled()`   | 작업이 cancel()로 취소되었는지 확인                                                       |
| `isDone()`        | 작업 완료 여부 확인 (정상, 취소, 예외 포함)                                                   |
| `get()`           | 결과가 나올 때까지 대기. 완료되면 결과 반환<br>예외: `InterruptedException`, `ExecutionException` |
...
→ 다른 작업이 끝날 때까지 현재 쓰레드를 대기하게 만듦

---

## Future 없이 결과를 받는다면?

* 쓰레드가 작업 끝날 때까지 기다려야 함
* 다른 작업 수행 불가능
* → **동시성 작업 불가**

### Future 덕분에 가능해진 것

* 요청 쓰레드가 대기하지 않고 다른 작업 수행 가능
* 필요한 모든 작업 요청 후 마지막에 `get()`으로 결과 받으면 됨
...
Integer result = future.get(); // 결과 받을 때까지 대기
```

* `submit()` 호출 즉시 Future 반환됨
* 나중에 필요할 때 `get()`으로 결과 받을 수 있음
...
* false: 실행 중인 작업은 그대로 둠 (인터럽트 안 함)
* 이후 `get()` 호출 시 `CancellationException` 발생
...
* 여러 `Callable` 작업을 한 번에 제출
* 모든 작업이 완료될 때까지 기다림

### invokeAny()

* 여러 `Callable` 중 **가장 먼저 완료된 작업 결과**만 반환
* 나머지 작업은 취소됨
...
* 쓰레드를 직접 생성하면 비용 큼, 관리 어려움, Runnable 불편함 있음
* Executor 프레임워크로 쓰레드 관리 간편하게 가능
* Callable, Future 통해 반환값, 예외 처리, 결과 대기 등 다양한 기능 제공
* Future는 작업 결과를 비동기로 받아올 수 있도록 도와줌
